...
重要なのは「独自のコンテキストウィンドウを使用する」という点です。
メインの会話とは別のコンテキストで動作し、作業結果の要約だけがメイン会話に返されます。
...
### サブエージェントはネストできない
サブエージェントから別のサブエージェントを生成できません。
ネストが必要なワークフローでは、メイン会話からサブエージェントをチェーンさせます。

### レイテンシを考慮する
サブエージェントは毎回新しいインスタンスとして起動し、コンテキストを収集する時間が必要です。
単純な作業には向きません。
