...
def build_roadmap_slide(prs):
    ...
    add_speaker_notes(slide,
        "全3回の最終回です。\n\n"
        "第1回では基本とコミュニケーション、第2回ではCLAUDE.mdとsettings.jsonによる"
        "セットアップを扱いました。\n\n"
...
def build_agenda_slide(prs):
    ...
    for i, a in enumerate(agenda):
        ...
        add_shape(slide, x, card_y, card_w, Emu(54864), fill=a["color"])

        add_textbox(slide, x + Emu(228600), card_y + Emu(182880), card_w - Emu(457200), Emu(365760),
                    a["part"], font=FONT_EN, size=Emu(203200), bold=True, color=a["color"])

        add_textbox(slide, x + Emu(228600), card_y + Emu(640080), card_w - Emu(457200), Emu(762000),
                    a["title"], font=FONT_JP, size=Emu(304800), bold=True, color=WHITE)

        add_divider(slide, x + Emu(228600), card_y + Emu(1554480), card_w - Emu(457200))

        add_textbox(slide, x + Emu(228600), card_y + Emu(1737360), card_w - Emu(457200), Emu(1200000),
                    a["desc"], font=FONT_JP, size=Emu(177800), color=BODY, line_spacing=4)
...
def build_section_slide(prs, part_num, title, subtitle, slide_num, notes=None):
...
def build_skill_intro_slide(prs):
    ...
    # Comparison table
    add_textbox(slide, Emu(731520), Emu(2800000), Emu(5000000), Emu(365760),
                "CLAUDE.mdとの違い", font=FONT_JP, size=Emu(228600), bold=True, color=WHITE)

    col_w = [Emu(2200000), Emu(4264480), Emu(4264480)]
    col_x = [Emu(731520)]
    for w in col_w[:-1]:
...
def build_skill_types_slide(prs):
    ...
    add_code_block(slide, lx, Emu(1650000), half_w, Emu(2400000),
                   "---\nname: api-conventions\ndescription: APIエンドポイント作成時の\n             設計パターン\n---\n\nAPIエンドポイントを書くときは以下に従う。\n- RESTfulな命名規則を使う\n- 一貫したエラーフォーマットを返す\n- リクエストバリデーションを含める")

    add_textbox(slide, lx, Emu(4150000), half_w, Emu(365760),
...
def build_skill_examples_slide(prs):
    ...
    add_code_block(slide, lx, Emu(1650000), half_w, Emu(2200000),
    ...
    add_textbox(slide, lx, Emu(3950000), half_w, Emu(274320),
                "使い方: /issue-worker 42",
    ...
    add_code_block(slide, rx, Emu(1650000), half_w, Emu(2200000),
                   "---\nname: pr-creator\ndescription: チームのテンプレートに沿って\n             PRを作成する\ndisable-model-invocation: true\n---\n\nPRを作成する。\n1. git diff で変更内容を確認\n2. 以下のテンプレートに沿って本文を作成\n   - ## 概要（何をしたか）\n   - ## 背景（なぜやるか）\n   - ## 影響範囲\n   - ## テスト結果\n3. gh pr create で提出")

    add_textbox(slide, rx, Emu(3950000), half_w, Emu(274320),
                "使い方: /pr-creator",
                font=FONT_CODE, size=Emu(177800), color=ACCENT2)
    ...
    add_speaker_notes(slide,
        ...
        "1つ目は issue-worker。Issue番号を渡すだけで、worktreeの作成からISSUE.mdの生成まで"
        "自動で行います。この後のPART 3で紹介するworktreeとの組み合わせで、複数Issueの並列開発が"
        "できるようになるので、ここは後でまた出てきます。\n\n"
...
def build_skill_design_slide(prs):
    ...
    col_w = [Emu(2800000), Emu(800000), Emu(7128960)]
    col_x = [Emu(731520)]
    for w in col_w[:-1]:
        col_x.append(col_x[-1] + w)
    ...
    for hdr, cx, cw in zip(headers, col_x, col_w):
        ...
        add_textbox(slide, cx + Emu(91440), hy + Emu(57150), cw - Emu(182880), Emu(274320),
    ...
    loc_col_w = [Emu(5364480), Emu(5364480)]
    loc_col_x = [Emu(731520), Emu(731520 + 5364480)]

    loc_headers = ["場所", "スコープ"]
    lhy = Emu(4350000)
    for hdr, cx, cw in zip(loc_headers, loc_col_x, loc_col_w):
        add_shape(slide, cx, lhy, cw, Emu(340000), fill=CARD_BG2)
        add_textbox(slide, cx + Emu(91440), lhy + Emu(57150), cw - Emu(182880), Emu(274320),
                    hdr, font=FONT_JP, size=Emu(165100), bold=True, color=ACCENT)
    ...
    for i, (loc, scope) in enumerate(locs):
        ry = lhy + Emu(340000) + Emu(i * 340000)
        for j, (cell, cx, cw) in enumerate(zip([loc, scope], loc_col_x, loc_col_w)):
            add_shape(slide, cx, ry, cw, Emu(330000), fill=CARD_BG if i % 2 == 0 else CARD_BG3)
            fn = FONT_CODE if j == 0 else FONT_JP
            fc = WHITE if j == 0 else BODY
            add_textbox(slide, cx + Emu(91440), ry + Emu(57150), cw - Emu(182880), Emu(274320),
                        cell, font=fn, size=Emu(152400), color=fc)

    add_footer(slide, "→ descriptionが鍵。1スキル1責務。大きくなったら分割する",
               y=Emu(5500000))
    ...
    add_speaker_notes(slide,
        ...
        "allowed-tools で使えるツールを制限できます。たとえばレビュー用スキルなら読み取り系ツールだけに絞る、"
        "といった使い方です。\n"
...
def build_builtin_commands_slide(prs):
    ...
    cmd_col_w = [Emu(3000000), Emu(7728960)]
    cmd_col_x = [Emu(731520), Emu(731520 + 3000000)]

    cmd_headers = ["コマンド", "用途"]
    chy = Emu(1500000)
    for hdr, cx, cw in zip(cmd_headers, cmd_col_x, cmd_col_w):
        add_shape(slide, cx, chy, cw, Emu(320000), fill=CARD_BG2)
        add_textbox(slide, cx + Emu(91440), chy + Emu(57150), cw - Emu(182880), Emu(274320),
                    hdr, font=FONT_JP, size=Emu(165100), bold=True, color=ACCENT)
    ...
    for i, (cmd, usage) in enumerate(commands):
        ry = chy + Emu(320000) + Emu(i * 290000)
        for j, (cell, cx, cw) in enumerate(zip([cmd, usage], cmd_col_x, cmd_col_w)):
            add_shape(slide, cx, ry, cw, Emu(280000), fill=CARD_BG if i % 2 == 0 else CARD_BG3)
            fn = FONT_CODE if j == 0 else FONT_JP
            fc = ACCENT if j == 0 else BODY
            add_textbox(slide, cx + Emu(91440), ry + Emu(45720), cw - Emu(182880), Emu(274320),
                        cell, font=fn, size=Emu(152400), color=fc)
    ...
    add_textbox(slide, Emu(731520), Emu(3600000), Emu(10000000), Emu(365760),
                "バンドルスキル（プロンプトベース）— エージェントを起動し複雑なタスクを遂行",
                font=FONT_JP, size=Emu(203200), bold=True, color=ACCENT2)

    sky = Emu(4000000)
    for hdr, cx, cw in zip(cmd_headers, cmd_col_x, cmd_col_w):
        add_shape(slide, cx, sky, cw, Emu(320000), fill=CARD_BG2)
        add_textbox(slide, cx + Emu(91440), sky + Emu(57150), cw - Emu(182880), Emu(274320),
                    hdr, font=FONT_JP, size=Emu(165100), bold=True, color=ACCENT2)
    ...
    for i, (sk, usage) in enumerate(skills):
        ry = sky + Emu(320000) + Emu(i * 290000)
        for j, (cell, cx, cw) in enumerate(zip([sk, usage], cmd_col_x, cmd_col_w)):
            add_shape(slide, cx, ry, cw, Emu(280000), fill=CARD_BG if i % 2 == 0 else CARD_BG3)
            fn = FONT_CODE if j == 0 else FONT_JP
    ...
    add_speaker_notes(slide,
        ...
        "ビルトインコマンドは、Claude Code本体に組み込まれた固定機能です。50以上あるので "
        "/help で確認してみてください。\n\n"
        "バンドルスキルはプロンプトベースで、エージェントを起動して複雑なタスクを遂行します。"
        "特に /batch と /simplify が強力です。この2つはPART 3のworktreeの話で改めて触れます。")
...
def build_subagent_intro_slide(prs):
    ...
    add_speaker_notes(slide,
        ...
        "前回学んだように、コンテキストは有限な資源でしたよね。大量のファイルを検索する作業をメインで"
        "やるとすぐにコンテキストが埋まりますが、サブエージェントに委譲すればメインのコンテキストを守れます。")
...
def build_subagent_combined_slide(prs):
    ...
    col_w = [Emu(2600000), Emu(2000000), Emu(6128960)]
    col_x = [Emu(731520)]
    for w in col_w[:-1]:
        col_x.append(col_x[-1] + w)
    ...
    add_textbox(slide, Emu(731520), Emu(2500000), Emu(10000000), Emu(274320),
                "→ Claudeが状況に応じて自動的に使い分ける",
                font=FONT_JP, size=Emu(165100), bold=True, color=ACCENT)

    add_divider(slide, Emu(731520), Emu(2900000), Emu(10728960))

    # Custom sub-agent
    add_textbox(slide, Emu(731520), Emu(3050000), Emu(10000000), Emu(365760),
                "カスタムサブエージェント — .claude/agents/ にMarkdownファイルを配置",
                font=FONT_JP, size=Emu(203200), bold=True, color=ACCENT2)

    add_code_block(slide, Emu(731520), Emu(3450000), Emu(6000000), Emu(2200000),
                   "---\nname: code-reviewer\ndescription: コードレビューの専門家。\n             コード変更後に積極的にレビュー。\ntools: Read, Grep, Glob, Bash\nmodel: sonnet\nmemory: project\n---\n\nあなたはシニアコードレビューアーです。\n\n呼び出されたときは以下を実行する。\n1. git diffで最近の変更を確認\n2. 変更されたファイルに焦点を当てる\n3. 即座にレビューを開始")

    # Key points on right
    points = [
        ("description", "Claudeが委譲先を\n判断する鍵", ACCENT),
        ("isolation: worktree", "独立したworktreeで\n動作させる", ACCENT2),
        ("memory: project", "セッションをまたいで\n知見を蓄積", ACCENT3),
    ]
    py = Emu(3450000)
    for label, desc, color in points:
        add_rounded_rect(slide, Emu(7000000), py, Emu(4460960), Emu(548640), fill=CARD_BG)
        add_shape(slide, Emu(7000000), py, Emu(54864), Emu(548640), fill=color)
        add_textbox(slide, Emu(7200000), py + Emu(45720), Emu(2000000), Emu(274320),
                    label, font=FONT_CODE, size=Emu(152400), bold=True, color=color)
        add_textbox(slide, Emu(9000000), py + Emu(45720), Emu(2300000), Emu(457200),
    ...
    add_speaker_notes(slide,
        "組み込みのサブエージェントが3つあります。Exploreは高速な検索特化型、Planはプランモード中の"
        "調査用、general-purposeは汎用タイプです。これらはClaudeが状況に応じて自動的に使い分けてくれます。\n\n"
        "カスタムのサブエージェントも作れます。.claude/agents/ にMarkdownファイルを置くだけです。\n\n"
        "大事なのがdescriptionです。たとえば「コード変更後に積極的にレビュー」と書いておけば、"
        "コード変更があったときに自動的にこのサブエージェントが呼ばれるようになります。\n\n"
        "isolation: worktree を追加すると、サブエージェントが独立したworktreeで動きます。\n"
        "memory: project を設定すると、サブエージェントがセッションをまたいで知見を蓄積してくれます。")
...
def build_hooks_slide(prs):
    ...
    # 3 code blocks
    examples = [
        ("例1: 編集後にlintを自動修正（PostToolUse）", ACCENT,
         '{\n  "matcher": "Edit|Write",\n  "hooks": [{\n    "type": "command",\n    "command": "npm run lint:fix"\n  }]\n}'),
        ("例2: テストを自動実行（PostToolUse）", ACCENT2,
         '{\n  "matcher": "Edit|Write",\n  "hooks": [{\n    "type": "command",\n    "command": "npm test -- --bail"\n  }]\n}'),
        ("例3: 完了通知（Notification）", ACCENT3,
         '{\n  "hooks": [{\n    "type": "command",\n    "command": "say \'完了しました\'"\n  }]\n}'),
    ]

    x_positions = [Emu(731520), Emu(4200000), Emu(7668480)]
    block_w = Emu(3200000)

    for (label, color, code), x in zip(examples, x_positions):
        add_textbox(slide, x, Emu(1950000), block_w, Emu(274320),
                    label, font=FONT_JP, size=Emu(152400), bold=True, color=color)
        add_code_block(slide, x, Emu(2300000), block_w, Emu(1500000), code)

    add_divider(slide, Emu(731520), Emu(4000000), Emu(10728960))

    add_textbox(slide, Emu(731520), Emu(4150000), Emu(10000000), Emu(365760),
                "ポイント: Hooksが失敗 → Claudeに通知 → Claudeが自動で修正を試みる",
                font=FONT_JP, size=Emu(203200), bold=True, color=WHITE)

    hook_points = [
        "ハンドラタイプ: command（シェル実行）、http、prompt、agent",
        "まずはcommandだけ覚えておけば十分",
    ]
    py = Emu(4600000)
...
def build_mcp_simple_slide(prs):
    ...
    jcol_w = [Emu(5364480), Emu(5364480)]
    jcol_x = [Emu(731520), Emu(731520 + 5364480)]

    jheaders = ["状況", "推奨"]
    jhy = Emu(3900000)
    for hdr, cx, cw in zip(jheaders, jcol_x, jcol_w):
        add_shape(slide, cx, jhy, cw, Emu(340000), fill=CARD_BG2)
        add_textbox(slide, cx + Emu(91440), jhy + Emu(57150), cw - Emu(182880), Emu(274320),
                    hdr, font=FONT_JP, size=Emu(165100), bold=True, color=ACCENT)
    ...
    for i, (situation, recommendation) in enumerate(judgments):
        ry = jhy + Emu(340000) + Emu(i * 330000)
        for j, (cell, cx, cw) in enumerate(zip([situation, recommendation], jcol_x, jcol_w)):
    ...
    add_speaker_notes(slide,
        ...
        "MCPは「CLIで足りないとき」の選択肢、と覚えておくのがいいかなと思います。設定方法は .mcp.json をリポジトリに置くだけです。")
...
def build_worktree_slide(prs):
    ...
    # CLI usage
    add_code_block(slide, Emu(731520), Emu(1650000), Emu(5200000), Emu(640080),
                   "claude --worktree feature-auth    # 自動でworktree作成+ブランチ切替\nclaude --worktree bugfix-payment  # 別ターミナルで並列実行")

    add_textbox(slide, Emu(731520), Emu(2400000), Emu(10000000), Emu(274320),
    ...
    # issue-worker flow
    add_textbox(slide, Emu(731520), Emu(2900000), Emu(10000000), Emu(365760),
                "/issue-worker スキルは --worktree にISSUE.md生成を足す",
                font=FONT_JP, size=Emu(203200), bold=True, color=WHITE)

    add_code_block(slide, Emu(731520), Emu(3350000), Emu(5800000), Emu(900000),
    ...
    add_textbox(slide, Emu(6800000), Emu(2900000), Emu(5000000), Emu(365760),
                "組み込みスキルもworktreeを活用",
                font=FONT_JP, size=Emu(203200), bold=True, color=ACCENT2)

    builtin_data = [
        ("/batch <指示>", "大規模変更を5〜30ユニットに\n分解、各worktreeで並列実行\n→ PR自動作成"),
        ("/simplify", "3つのレビューエージェントを\n並列起動、品質・効率・\n再利用性を検証"),
    ]

    by = Emu(3350000)
    for cmd, desc in builtin_data:
        add_rounded_rect(slide, Emu(6800000), by, Emu(4660960), Emu(457200), fill=CARD_BG)
        add_textbox(slide, Emu(6900000), by + Emu(68580), Emu(1800000), Emu(274320),
                    cmd, font=FONT_CODE, size=Emu(152400), bold=True, color=ACCENT2)
        add_textbox(slide, Emu(8700000), by + Emu(68580), Emu(2600000), Emu(365760),
                    desc, font=FONT_JP, size=Emu(127000), color=BODY)
        by += Emu(500000)

    add_footer(slide,
               "→ カスタム（/issue-worker）と組み込み（/batch）を使い分け。設定はすべてのworktreeに引き継がれる",
               y=Emu(4600000))
    add_page_num(slide, 17)

    add_speaker_notes(slide,
        "Claude Codeには --worktree フラグが組み込まれています。claude --worktree feature-auth "
        "のように起動するだけで、自動的にworktreeが作成されて、独立した環境でセッションが始まります。\n\n"
        "--worktree だけでも並列作業はできるんですが、Issue番号との紐付けやISSUE.mdの自動生成まではやってくれません。"
        "そこで、PART 1で紹介した /issue-worker の出番です。\n\n"
        "/batch は大規模な変更を自動で分解し、各ユニットをworktree隔離で並列実行してPRまで作ってくれます。\n"
        "/simplify は実装後のコードレビュー用です。3つのレビューエージェントが並列で品質をチェックします。\n\n"
        "大事なポイントとして、--worktree で作られた環境にはCLAUDE.md、Skills、Hooks、サブエージェントの"
        "設定がすべて引き継がれます。")


def build_overview_slide(prs):
    """Slide 18: Overview of large-scale development."""
    slide = new_slide(prs)
    set_bg(slide, BG_MAIN)
    ...
    # Overview table
    col_w = [Emu(2400000), Emu(4264480), Emu(4064480)]
    col_x = [Emu(731520)]
    for w in col_w[:-1]:
        col_x.append(col_x[-1] + w)
    ...
    for i, (name, role, loc) in enumerate(overview_rows):
        ry = hy + Emu(340000) + Emu(i * 310000)
    ...
    add_divider(slide, Emu(731520), Emu(3350000), Emu(10728960))

    # Flow steps
    add_textbox(slide, Emu(731520), Emu(3500000), Emu(10000000), Emu(365760),
                "大規模開発のフロー", font=FONT_JP, size=Emu(228600), bold=True, color=WHITE)

    steps = [
        ("1", "セットアップ", "CLAUDE.md / Skills / Hooks /\nサブエージェント / MCP を設定", ACCENT),
        ("2", "Issue割り当て", "/issue-worker で\nworktree + ISSUE.md を作成", ACCENT2),
        ("3", "並列実行", "各worktreeでClaude Codeを\n起動し、独立して作業を進める", ACCENT3),
        ("4", "品質担保", "Hooksが自動でlint・テスト", RGBColor(0xCC, 0xAA, 0x55)),
        ("5", "完了", "PR作成 → 人間がコードレビュー\n→ マージ → worktree削除", MUTED2),
    ]
    ...
    for i, (num, title, desc, color) in enumerate(steps):
        sx = Emu(731520) + i * (step_w + step_gap)
        add_rounded_rect(slide, sx, sy, step_w, Emu(1800000), fill=CARD_BG)
        add_shape(slide, sx, sy, step_w, Emu(54864), fill=color)

        num_s = slide.shapes.add_shape(MSO_SHAPE.OVAL, sx + Emu(91440), sy + Emu(137160),
        ...
        run = tf.paragraphs[0].add_run()
        run.text = num
        _set_font(run, FONT_EN, Emu(190500), True, WHITE)

        add_textbox(slide, sx + Emu(548640), sy + Emu(137160), step_w - Emu(640080), Emu(365760),
                    title, font=FONT_JP, size=Emu(165100), bold=True, color=color)

        add_textbox(slide, sx + Emu(91440), sy + Emu(640080), step_w - Emu(182880), Emu(1097280),
                    desc, font=FONT_JP, size=Emu(127000), color=BODY, line_spacing=3)
    ...
    add_speaker_notes(slide,
        ...
        "各worktreeでClaude Codeを起動し、独立して作業を進めます。Hooksが自動でチェックし、"
        "最後にPR作成。自動チェックはレビュー負荷を下げるためのもので、最終的なコードレビューと"
        "マージ判断は人間が行います。")
...
def build_summary_slide(prs):
    ...
    # Series review table
    add_divider(slide, Emu(731520), Emu(4300000), Emu(10728960))

    review_col_w = [Emu(1400000), Emu(2600000), Emu(6728960)]
    review_col_x = [Emu(731520)]
    for w in review_col_w[:-1]:
        review_col_x.append(review_col_x[-1] + w)

    rhy = Emu(4450000)
    r_headers = ["回", "テーマ", "学んだこと"]
    for hdr, cx, cw in zip(r_headers, review_col_x, review_col_w):
        add_shape(slide, cx, rhy, cw, Emu(300000), fill=CARD_BG2)
        add_textbox(slide, cx + Emu(91440), rhy + Emu(45720), cw - Emu(182880), Emu(274320),
    ...
    for i, (num, theme, learned) in enumerate(review_rows):
        ry = rhy + Emu(300000) + Emu(i * 290000)
        for j, (cell, cx, cw) in enumerate(zip([num, theme, learned], review_col_x, review_col_w)):
            add_shape(slide, cx, ry, cw, Emu(280000), fill=CARD_BG if i % 2 == 0 else CARD_BG3)
            fc = WHITE if j <= 1 else BODY
            fb = j <= 1
            add_textbox(slide, cx + Emu(91440), ry + Emu(45720), cw - Emu(182880), Emu(274320),
                        cell, font=FONT_JP, size=Emu(152400), bold=fb, color=fc)

    add_textbox(slide, Emu(731520), Emu(5700000), Emu(10728960), Emu(457200),
    ...
    add_speaker_notes(slide,
        "まとめです。今日の持ち帰りは3つ。\n\n"
        "1つ目。SkillsとサブエージェントでClaudeの能力を拡張する。軽い作業はスキル、"
        "重い探索はサブエージェント。手順書とチームメンバーの違いでしたね。\n\n"
...
def main():
    ...
    build_section_slide(prs, 1, "Skills & サブエージェント",              # 04
                        "繰り返しをなくし、分業する", 4,
                        notes="では早速PART 1に入りましょう。Skillsとサブエージェントです。\n\n"
                        "この2つは結構つながりが深くて、Skillsで指示を仕組み化し、サブエージェントで"
                        "重い作業を委譲する。どちらもClaudeの能力を拡張する仕組みなんですが、"
                        "使いどころが違います。そのあたりを順番に見ていきます。")
    ...
    build_section_slide(prs, 2, "Hooks & MCP", "自動化し、つなぐ", 13,   # 13
                        notes="ここからPART 2です。HooksとMCPを扱います。\n\n"
    ...
    build_section_slide(prs, 3, "worktree",                               # 16
                        "すべてを束ねて、並列で挑む", 16,
                        notes="PART 3、worktreeです。ここが今日の話の集大成になります。\n\n"
